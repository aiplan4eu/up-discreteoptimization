...
from discrete_optimization.rcpsp.rcpsp_parser import RCPSPModel, parse_file
from discrete_optimization.rcpsp.rcpsp_utils import create_fake_tasks
from unified_planning.model.scheduling import SchedulingProblem
from unified_planning.shortcuts import LE, Equals
...
def from_do_to_up(rcpsp_problem: RCPSPModel) -> SchedulingProblem:
    ...
    for resource in rcpsp_problem.resources_list:
        resources[resource] = problem.add_resource(
            resource, rcpsp_problem.get_max_resource_capacity(resource)
        )
    # Initialize tasks and resource consumptions.
    activities = {}
    for activity in rcpsp_problem.tasks_list:
        activities[activity] = problem.add_activity(
            name=str(activity),
        ...
        for resource in rcpsp_problem.mode_details[activity][1]:
            ...
            if resource in resources:
                activities[activity].uses(
                    resources[resource],
                    rcpsp_problem.mode_details[activity][1][resource],
                )
    ...
    for activity in rcpsp_problem.successors:
        for next_activity in rcpsp_problem.successors[activity]:
            problem.add_constraint(
                LE(activities[activity].end, activities[next_activity].start)
            )
    # "Special constraints"
    ...
    if rcpsp_problem.do_special_constraints:
        for activity in rcpsp_problem.special_constraints.end_times_window:
            up_value_end = rcpsp_problem.special_constraints.end_times_window[activity][
                1
            ]
        ...
        for activity in rcpsp_problem.special_constraints.start_times_window:
            low_value_start = rcpsp_problem.special_constraints.start_times_window[
                activity
            ][0]
        ...
        for ac1, ac2, lag in rcpsp_problem.special_constraints.start_after_nunit:
            problem.add_constraint(
                LE(activities[ac1].start + lag, activities[ac2].start)
            )
    ...
    resource_consumption_data: List[Dict[str, int]] = create_fake_tasks(
